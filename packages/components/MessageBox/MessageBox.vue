...
import SwdOverlay from "../Overlay/Overlay.vue";
import SwdIcon from "../Icon/Icon.vue";
import SwdButton from "../Button/Button.vue";
import SwdInput from "../Input/Input.vue";

import { useId, useZIndex } from "@swd-design/hooks";
import { typeIconMap } from "@swd-design/utils";

defineOptions({ name: "SwdMessageBox", inheritAttrs: false });
...
<template>
  <transition name="fade-in-linear" @after-leave="destroy">
    <swd-overlay v-show="(visible as Ref).value" :z-index="state.zIndex" mask>
      <div
        role="dialog"
        class="swd-overlay-message-box"
      ...
      >
        ...
        >
          <div
            ...
            ref="headerRef"
            class="swd-message-box__header"
            :class="{ 'show-close': state.showClose }"
          >
            <div class="swd-message-box__title">
              <swd-icon
                v-if="iconComponent && state.center"
                :class="{
                  [`swd-icon-${state.type}`]: state.type,
                }"
                :icon="iconComponent"
              />
              {{ state.title }}
            ...
            <button
              v-if="showClose"
              class="swd-message-box__header-btn"
              @click.stop="handleClose"
            >
              <swd-icon icon="xmark" />
            </button>
          </div>
          <div class="swd-message-box__content">
            <swd-icon
              ...
              :icon="iconComponent"
            />
            <div v-if="hasMessage" class="swd-message-box__message">
              <slot>
                <component
          ...
          <div v-show="state.showInput" class="swd-message-box__input">
            <swd-input
