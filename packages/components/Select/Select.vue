...
import { RenderVnode, debugWarn } from "@swd-design/utils";
import { useFocusController, useClickOutside } from "@swd-design/hooks";
...
import SwdOption from "./Option.vue";

const COMPONENT_NAME = "SwdSelect" as const;
defineOptions({
  name: COMPONENT_NAME,
});
...
const children = computed(() =>
  filter(slots?.default?.(), (child) => eq(child.type, SwdOption))
);
...
<template>
  ...
  >
    ...
    >
      ...
      <template #content>
        <div class="swd-select__loading" v-if="selectStates.loading">
          <swd-icon icon="spinner" spin />
        </div>
        <div class="swd-select__nodata" v-else-if="filterable && isNoData">
          No data
        </div>
        <ul class="swd-select__menu" v-else>
