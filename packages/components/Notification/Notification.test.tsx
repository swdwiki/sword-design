...
describe("createMessage", () => {
  test("call notification()", async () => {
    ...
    await rAF();
    expect(document.querySelector(".swd-notification")).toBeTruthy();
    handler.close();
    await rAF();
    expect(document.querySelector(".swd-notification")).toBeFalsy();
  });
  ...
  test('call notification() more times',async()=> {
    ...
    notification({ message: "hello msg", duration: 0 });
    await rAF();
    expect(document.querySelectorAll(".swd-notification").length).toBe(2);
    notification.closeAll()
    await rAF();
    expect(document.querySelectorAll(".swd-notification").length).toBe(0);
  })
  ...
  test('offset',async()=>{
    ...
    await rAF()
    const elements = document.querySelectorAll(".swd-notification")
