...
<template>
  <ul class="swd-upload-list">
    <li v-for="item in fileList" :key="item.uid" class="swd-upload-list__item">
      <span class="file-name" :class="{ [`file-name-${item.status}`]: true }">
        <swd-icon icon="file-alt" />
        {{ item.name }}
      </span>
      <span
        class="file-status"
      ...
      <span class="file-action">
        <swd-icon icon="times" @click="onRemove?.(item)" />
      </span>
    </li>
  </ul>
</template>
